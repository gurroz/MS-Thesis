...
string PairMultiplyShift::hash(int list[], int length) {
    unsigned long long hashVal1 = 17;
    for(int i = 0; i < (length / 2); i++) {
        int firstIndex = 2*i;
        int secondIndex = firstIndex + 1;

        unsigned long long mult1 = (a1Constant[firstIndex] + list[secondIndex]) * (a1Constant[secondIndex] + list[firstIndex]);
//        long mult2 = (a2Constant[firstIndex] + list[secondIndex]) * (a2Constant[secondIndex] + list[firstIndex]);
